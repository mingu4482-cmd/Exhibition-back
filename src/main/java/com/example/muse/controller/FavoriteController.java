...
public class FavoriteController {
    ...
    // ✅ 찜 추가: POST /api/favorites?performanceId=388
    @PostMapping
    public Map<String, Object> add(@RequestParam Long performanceId, Authentication auth) {
        String email = (String) auth.getPrincipal();
        favoriteService.addFavorite(email, performanceId);
        return Map.of("ok", true);
    }

    // ✅ 찜 삭제: DELETE /api/favorites?performanceId=388
    @DeleteMapping
    public Map<String, Object> remove(@RequestParam Long performanceId, Authentication auth) {
        String email = (String) auth.getPrincipal();
        favoriteService.removeFavorite(email, performanceId);
        return Map.of("ok", true);
    }
